...
def _ask_yes_no(prompt: str, model: str = JUDGE_MODEL,
                task: str = "judge") -> tuple[bool, int]:
...
def judge_faithfulness(answer: str, context_chunks: list[str]) -> tuple[float, int]:
    ...
    yes, tokens = _ask_yes_no(prompt, task="judge_faithfulness")
...
def audit_correctness(question: str, pred: str, gold: str) -> tuple[float, int]:
    ...
    if not pred.strip():
        return 0.0, 0
    yes, tokens = _ask_yes_no(_correctness_prompt(question, pred, gold),
                              model=AUDIT_MODEL, task="audit_correctness")
    return (1.0 if yes else 0.0), tokens
