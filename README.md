...
| `dataset.py` | **自訂資料集**：你的 .txt/.md 文件 → 段落語料 → LLM 生成可抽取式 QA（或自備 QA）→ 沿用整條評估/最佳化管線 |
...
```powershell
# 1. 用 LLM 對你的文件生成 QA 評估集 (一次性；.txt/.md 檔或資料夾皆可)
...
python run.py   --corpus docs/ --qa data/qa.jsonl
python plot.py
```
...
- 中文文件可用，但 EM/F1 對中文參考性低，以 LLM 判官的 correctness 為準。
