...
| `distill.py` | 把專案的 LLM 呼叫蒐集/蒸餾成可離線跑的小模型（LoRA SFT，需 GPU；資料蒐集/篩選不需要） |
...
| `sweep.py` | 核心網格掃描（3 chunk_size × 3 top_k × 4 retriever = 36 組）→ 填 cache + go/no-go（成對符號檢定） |
...
| `run.py` | 編排 random vs bandit vs OPRO vs hybrid 對照（CLI） |
...
python sweep.py   # 2. 核心網格 (3 chunk_size × 3 top_k × 4 retriever = 36 組) 掃描填 cache，看 go/no-go
python run.py     # 3. random vs bandit vs OPRO vs hybrid 對照 (分數查 cache，僅 meta 呼叫大模型)
...
以下是 2026-07 用 NVIDIA NIM 真實跑出來的結果（30 題 HotpotQA，random vs OPRO，budget=10）：

![對比曲線與 Pareto 前緣範例](assets/comparison-example.png)

左圖：OPRO 在第 6 次評估後找到 `retriever=graph, rerank=True, query_decompose=True` 的配置，
objective 從 random 卡住的 0.667 跳到 0.717 並保持——**正確率打平（0.80）、但幻覺率從 27% 降到 17%**，
是 OPRO 靠失敗分群推理找到更好權衡點的具體證據，不是隨機運氣。
右圖：Pareto 前緣正確地往左上角（低幻覺、高正確率）凸出，點大小是每題 token 成本。
這一輪裡 random 的 10 組有 9 組被 racing 提早停止，只有暖身組跑滿——真實條件下省下的評估成本很可觀。

> 圖表僅供參考：這是單一 seed、budget=10 的一次性結果，不是統計上顯著的結論
> （要驗證「OPRO 系統性贏 random」得跑 `--seeds` 多組，見上方成本提醒）。
...
## 本地/離線模型（蒸餾，適合有 GPU 的環境）

想擺脫 API 依賴（成本、延遲、資料外送顧慮），可以把這個專案「餵過的 LLM 呼叫」蒐集起來，
蒸餾成一個能離線跑的小模型。全部呼叫都經過 `rag.py` 的 `api_call()` 這一個出口，
設一個環境變數就能開始蒐集，不用另外寫資料生成腳本：

```powershell
# 1. 蒐集: 照平常跑 eval.py / sweep.py / run.py / server.py，順便留下教材
$env:DISTILL_LOG_PATH = "data/distill.jsonl"
python eval.py
python sweep.py
python run.py

# 2. 看蒐集到什麼 (不需要 GPU)
python distill.py stats --log data/distill.jsonl

# 3. 篩選單一任務 + 轉成 SFT 格式 (不需要 GPU)
#    task 可選: generate / judge_correctness / judge_faithfulness / decompose /
#    hyde / gap_query / compress / opro_meta / hybrid_region / qa_gen / qa_gen_multihop
python distill.py export --log data/distill.jsonl --task generate --out data/sft_generate.jsonl

# 4. LoRA 微調 (需要 GPU；沒有 CUDA 會直接報錯提示，不會卡住)
pip install -r requirements-distill.txt
python distill.py train --data data/sft_generate.jsonl `
    --base-model Qwen/Qwen2.5-1.5B-Instruct --out distilled/generate

# 5. 用 llama.cpp 把 base model + LoRA adapter 合併、轉成 GGUF (llama.cpp 環境操作，
#    非本專案範圍；參考 llama.cpp 的 convert_hf_to_gguf.py + merge_lora)
python distill.py modelfile --gguf my-model.gguf --out Modelfile
ollama create my-distilled -f Modelfile

# 6. 讓整條既有管線指向本地模型 (不用改任何程式碼)
#    .env: BASE_URL=http://localhost:11434/v1
#          GEN_MODEL=my-distilled
#          NVIDIA_API_KEY=ollama   # Ollama 不驗證，但 openai client 要求非空字串
python eval.py   # 完全離線跑
```

設計上刻意**不**把所有任務混成一個模型：每筆訓練樣本都標記來源任務（`distill_task`），
生成、判官、HyDE、OPRO 推理這些行為性質差很多——判官只需要學會穩定輸出 YES/NO，
生成器要學會摘要式簡答，OPRO 推理要學會結構化 JSON 輸出。混在一起蒸餾，小模型會
在三者間互相拉扯；`export --task` 讓你可以只挑一種行為蒸餾出專精的小模型（例如先蒸餾
判官，省下評估流程裡佔最多次呼叫的那一塊）。

> ⚠️ 這一步需要 GPU（LoRA SFT），這台開發機是純 CPU 環境沒有實跑驗證，
> 但資料蒐集（`api_call` 鉤子）、篩選/去重/匯出、GPU 前置檢查都已離線測試覆蓋
> （見 `tests/test_distill.py`）。`stats`/`export`/`modelfile` 三個子命令完全不需要 GPU。
...
- [x] 蒸餾：LLM 呼叫蒐集鉤子（`distill_task` 標記）+ export/train/modelfile CLI，
      `BASE_URL` 覆寫讓既有管線可指向本地模型（訓練步驟需 GPU，這台開發機未實跑驗證）

> 2026-07 已用 NVIDIA NIM 真實跑過一輪端到端流程（`rag.py`→`eval.py`→`sweep.py`→`run.py`→
> `plot.py`），見上方「實際跑一輪長什麼樣」；過程中修掉幾個只有真的跑過才會發現的 bug
> （HotpotQA 分層抽樣、client timeout、rerank 端點永久性錯誤處理）。
